...
function openModal(productId) {
  ...
  const { name, cat, desc, price, emoji, image, badge } = product;

  // Imagen o emoji
  const mediaSrc = image || null;
  ...
  const imgHTML = mediaSrc
    ? `<img src="${mediaSrc}" alt="${name}" id="modalImg" onclick="toggleZoom()" title="Click para hacer zoom">`
    : `<div class="modal-emoji">${emoji}</div>`;

  // Características (extraídas de la descripción como bullets)
  ...
  document.getElementById('modalContent').innerHTML = `
    <div class="modal-img-wrap" id="modalImgWrap">
      ${imgHTML}
      ${badgeHTML}
      <button class="modal-zoom-hint" id="zoomHint">🔍 Click para zoom</button>
    </div>
    <div class="modal-info">
      <span class="modal-cat">${CAT_LABELS[cat] || cat}</span>
      <h2 class="modal-name">${name}</h2>
      <p class="modal-price"><sup>$</sup>${price}</p>
      ...
      <div class="modal-actions">
        <button class="modal-add-btn" onclick="addToCart(${productId}); showToast('🛒 ${name.replace(/'/g, "\\'")} agregado'); closeModal();">
          🛒 Agregar al carrito
        </button>
...
function toggleZoom() {
  ...
  if (currentZoom) {
    ...
    hint.textContent = '🔍 Click para alejar';
  } else {
    img.classList.remove('zoomed');
    hint.textContent = '🔍 Click para zoom';
  }
}
